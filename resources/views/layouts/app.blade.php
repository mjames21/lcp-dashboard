...
{{-- Single-file configurable layout. Edit only the $ui array to change title, logo, footer, and nav. --}}
@php
            ...
            [
                ...
                'children' => [
                    [
                        ...
                        'params'=> ['code' => 'Health'],
                        'active'=> ['sector/Health', 'sector.dashboard'],
                    ],
                    [
                        'type'  => 'link',
                        'label' => 'Education',
                        'route' => 'sector.dashboard',
                        'params'=> ['code' => 'Education'],
                        'active'=> ['sector/Education', 'sector.dashboard'],
                    ],
                    [
                        'type'  => 'link',
                        'label' => 'Agriculture',
                        'route' => 'sector.dashboard',
                        'params'=> ['code' => 'Agriculture'],
                        'active'=> ['sector/Agriculture', 'sector.dashboard'],
                    ],
                    [
                        'type'  => 'link',
                        'label' => 'Finance',
                        'route' => 'sector.dashboard',
                        'params'=> ['code' => 'Finance'],
                        'active'=> ['sector/Finance', 'sector.dashboard'],
                    ],
                ],
            ],
            [
    ...
    $isActive = function(array $item): bool {
        ...
        foreach ($patterns as $p) {
            // URL-style patterns (contain '/' or '*') use request()->is()
            if (str_contains($p, '/') || str_contains($p, '*')) {
                if (request()->is($p)) return true;
                continue;
            }
            // Route name match; if params defined, require they match too
    ...
    $renderNav = function(array $items, int $depth = 0) use (&$renderNav, $hrefFor, $isActive, $anyActive): string {
        ...
        foreach ($items as $it) {
            ...
            $label    = $it['label'] ?? 'Item';
            $icon     = $it['icon']  ?? null;
            $title    = $it['title'] ?? $label; // tooltip when mini
            ...
            if ($type === 'group') {
                ...
                // indent container (match original)
                $pad = $depth === 0 ? 'pl-10' : 'pl-6';
                $out .= '<div x-show="open && !sidebarMini" x-collapse class="'.$pad.' pr-2 space-y-1">';
            ...
            // link
            $active = $isActive($it);
            // only highlight submenu (depth > 0). top-level stays neutral
            $state  = ($depth > 0 && $active)
                ? 'bg-gray-200 text-gray-800 font-semibold'  // submenu active
                : 'text-gray-700 hover:bg-gray-100';          // default
            $href   = $hrefFor($it);
            $target = $external ? ' target="_blank" rel="noopener noreferrer"' : '';

            if ($depth > 0 && empty($icon)) {
                $out .= '<a href="'.e($href).'" class="block px-3 py-2 rounded-md '.$state.'" :title="sidebarMini ? $el.dataset.tt : \"\"" data-tt="'.e($title).'"'.$target.'>'.e($label);
                if ($external) {
                    $out .= ' <i class="fa-solid fa-arrow-up-right-from-square text-xs ml-1"></i>';
                }
                $out .= '</a>';
            } else {
                $out .= '<a href="'.e($href).'" class="flex items-center gap-3 px-3 py-2 rounded-md transition-colors '.$state.'" :title="sidebarMini ? $el.dataset.tt : \"\"" data-tt="'.e($title).'"'.$target.'>';
                $out .= '<i class="fa-solid '.e($icon ?? 'fa-circle-dot').' w-5 text-gray-900"></i>';
                $out .= '<span x-show="!sidebarMini">'.e($label).'</span>';
                if ($external) {
                    $out .= '<i x-show="!sidebarMini" class="fa-solid fa-arrow-up-right-from-square text-xs ml-1"></i>';
                }
                $out .= '</a>';
            }
        }
...
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-data="{ sidebarOpen: false, sidebarMini: false }" x-init="sidebarMini = JSON.parse(localStorage.getItem('lcpd.sidebarMini') ?? 'false')" @toggle-sidebar.window="sidebarOpen = !sidebarOpen" @toggle-sidebar-mini.window="sidebarMini = !sidebarMini; localStorage.setItem('lcpd.sidebarMini', JSON.stringify(sidebarMini))">
<head>
    <meta charset="utf-8" />
...
<aside class="w-64 bg-white text-black h-screen flex flex-col border-r shadow-sm" :class="[
    ...
    <div class="h-14 flex items-center justify-center border-b">
        <img src="{{ asset($ui['logo_path'] ?? 'img/switch-logo.jpg') }}" class="{{ $ui['logo_class'] ?? 'h-8' }}" alt="{{ $ui['logo_alt'] ?? 'Logo' }}">
    </div>
